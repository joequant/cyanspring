...
public class IbOrderTest implements IDownStreamListener {
	...
	public void testDownStream() throws InterruptedException {
		...
		assertTrue(order.getOrdStatus().equals(OrdStatus.NEW) ||
		...
		if(!order.getOrdStatus().equals(OrdStatus.FILLED)) {
			...
			assertTrue(order.getOrdStatus() == OrdStatus.REPLACED);
			assertTrue(PriceUtils.Equal(order.getPrice(), 21.0));
		}
		
		//test reject
		order = new ChildOrder("C", OrderSide.Buy, 0, 28, ExchangeOrderType.LIMIT, "", "");
		try {
			sender.newOrder(order);
		} catch (DownStreamException e) {
			log.debug(e.getMessage(), e);
		...
		assertTrue(order.getOrdStatus().equals(OrdStatus.REJECTED));
